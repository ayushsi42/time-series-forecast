...
class MaskedMetric(Metric):
    ...
    def __init__(self,
                 ...
                 metric_kwargs=None,
                 at=None):
        metric_init_params = inspect.signature(Metric.__init__).parameters
        if 'compute_on_step' in metric_init_params:
            ...
            super(MaskedMetric, self).__init__(dist_sync_on_step=dist_sync_on_step,
                                               process_group=process_group,
                                               dist_sync_fn=dist_sync_fn)
            # torchmetrics>=1.4 removed the argument but still expects the attribute
