...
class TemporalFeaturesMixin:
    ...
    def datetime_encoded(self, units):
        ...
        def _unit_to_ns(unit: str) -> int:
            td = pd.to_timedelta('1' + unit)
            delta = getattr(td, 'delta', None)
            if delta is not None:
                return delta
            # pandas >=2.3 removed .delta; convert to integer nanoseconds
            return int(td / pd.Timedelta('1ns'))

        mapping = {un: _unit_to_ns(un)
                   for un in ['day', 'hour', 'minute', 'second',
                              'millisecond', 'microsecond', 'nanosecond']}
        mapping['week'] = _unit_to_ns('W')
